...
public class HandVisual : MonoBehaviour
{
	...
	// burn a card
	/* TODO
	public void BurnACard(CardAsset c, int UniqueID, bool fast = false)
	{
		// Debug.Log ("In HandVisual - BurnACard");
		GameObject card = CreateACardAtPosition(c, DeckTransform.position, new Vector3(0f, -179f, 0f));
		card.GetComponent<WhereIsTheCardOrCreature>().VisualState = VisualStates.Transition;
		// move card to draw review spot
		Sequence s = DOTween.Sequence();
		if (!fast)
		{
			// Debug.Log ("Not fast!!!");
		...
		s.OnComplete(()=>
			{
				GameObject explosion = 
					Instantiate(GlobalSettings.Instance.ExplosionPrefab, DrawPreviewSpot.position, Quaternion.identity);
				Sequence t = DOTween.Sequence();
				t.PrependInterval(2f);
				t.OnComplete(()=>
					{
		...
		// Debug.Log ("Finish HandVisual - BurnACard");
	} */
    ...
    public void PlayASpellFromHand(GameObject CardVisual)
    {
		...
		CardVisual.GetComponent<OneCardManager>().CardFaceGlowImage.enabled = false;
